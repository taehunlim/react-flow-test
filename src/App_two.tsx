...
import { nodes as initialNodes, edges as initialEdges } from './nodes2';
...
const App = () => {
   ...
   const onNodeDragStart = (evt_: any, node: Node) => {
      dragRef.current = node;
   };

   const onNodeDrag = (evt_: any, node: Node) => {
         ...
         const targetNode = nodes.find((n) => {
            if (n.width && n.height) {
               return (
                  ...
                  n.id !== node.id // this is needed, otherwise we would always find the dragged nod
               );
            }
         });
   ...
   const onNodeDragStop = (evt_: any, node: Node) => {
